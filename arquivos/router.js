...
const db = require("./db");
//const SESSION_TIMEOUT = 30 * 60 * 1000; // Exemplo de tempo limite de sessão (30 minutos)
...
router.post("/login", (req, res) => {
    ...
    db.query("SELECT * FROM usuarios WHERE email = ?", [email], (err, result) => {
        if (err) {
            return res.status(500).send({ msg: "Erro no servidor ao buscar o usuário." });
        }

        if (result.length > 0) {
            const user = result[0];

            // Verificação do status de login e inatividade
            if (user.logged_in === 1) {
            ...
            bcrypt.compare(senha, user.senha, (error, response) => {
                if (error) return res.status(500).send({ msg: "Erro ao comparar as senhas." });
